    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex flex-col sm:flex-row items-start gap-6 mb-10">
        <img
          ...
          alt={author.name}
          className="w-20 h-20 rounded-full bg-tangent-warm"
          width={80}
          height={80}
        />
        <div>
          <h1 className="text-3xl font-black tracking-tight">{author.name}</h1>
          <p className="mt-2 text-tangent-slate leading-relaxed">{author.bio}</p>
          <p className="mt-2 text-sm text-tangent-slate">
      ...
      {/* Articles by author */}
      <div className="divide-y divide-tangent-border">
        {articles.map((article) => (
          <article key={article.id} className="py-5 first:pt-0">
            <Link href={`/article/${article.slug}`} className="group block no-underline">
              <h2 className="text-lg font-bold text-tangent-black group-hover:text-accent-primary transition-colors">
                {article.title}
              </h2>
              <p className="mt-1 text-sm text-tangent-slate line-clamp-2">{article.excerpt}</p>
              <div className="mt-2 flex items-center gap-2 text-xs text-tangent-slate">
      ...
      {articles.length === 0 && (
        <p className="text-tangent-slate text-center py-12">কোনো প্রতিবেদন পাওয়া যায়নি।</p>
      )}
